...
def get_year_statistics(file_name, vacancy, q, currencies):
...
def get_all_years_statistics():
    ...
    for file_name in os.listdir(temp):
        p = Process(target=get_year_statistics, args=(temp + '/' + file_name, user_input.vacancy, q, currencies.copy()))
        process.append(p)
        p.start()
    for p in process:
        p.join(1)
...
if __name__ == '__main__':
    ...
    first_sheet_data = [st.years_salary, st.years_salary_vac, st.years_count,
                        st.years_count_vac]
    first_table_data = [st.years_salary, st.years_salary_vac, st.years_count,
                        st.years_count_vac]
    ...
    Report.generate_image([st.years_salary, st.years_salary_vac], user_input.vacancy,
                               [st.years_count, st.years_count_vac]
    ...
    Report.generate_pdf(user_input.vacancy, first_table_data,
                             ['Год', 'Средняя зарплата', 'Средняя зарплата - {0}'.format(user_input.vacancy),
                              'Количество вакансий', 'Количество вакансий - {0}'.format(user_input.vacancy)],
