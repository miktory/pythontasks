...
Все скрипты генерируют отчёт в один и тот же файл ***report.pdf***, отчёты для этого задания я скопировал и сохранил под именем ***second_report.pdf***  
На elearn файлы по какой-то причине перестали загружаться, поэтому вы можете загрузить отчёт из этого репозитория.
Pdf отчёт находится в файле report.pdf.  
