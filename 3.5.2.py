...
def edit_salary(date, salary_from, salary_to, salary_currency, cursor):
    ...
    if salary_currency in ["BYN", "BYR", "EUR", "KZT", "UAH", "USD"]:
        ...
        currency_exchange = cursor.execute("select * from currencies where date == :date", {"date": date}).fetchone()[currencies_num[salary_currency]]
...
df = pd.read_csv(input("Введите название файла (vacancies_dif_currencies.csv): "))
